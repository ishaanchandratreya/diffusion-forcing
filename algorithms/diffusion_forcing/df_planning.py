...
class DiffusionForcingPlanning(DiffusionForcingBase):
    ...
    def _build_model(self):
        ...
        mean = list(self.observation_mean) * self.repeat_observation + list(
            self.action_mean
        )
        std = list(self.observation_std) * self.repeat_observation + list(
            self.action_std
        )
        if self.use_reward:
    ...
    def configure_optimizers(self):
        transition_params = tuple(self.transition_model.parameters()) + tuple(
            self.init_encoder.parameters()
        )
    ...
    def _preprocess_batch(self, batch):
        ...
        rewards = rearrange(
            rewards, "b (t fs) -> t b fs 1", fs=self.frame_stack
        ).contiguous()

        init_obs = observations[:, 0]
        init_z = self.init_encoder(
            self.split_bundle(self._normalize_x(self.make_bundle(init_obs)))[0]
        )
        observations = observations[:, 1:]
        pad = torch.zeros_like(observations[:, -1:])
        observations = torch.cat([observations, pad], 1)
        observations = rearrange(
            observations, "b (t fs) ... -> t b fs ...", fs=self.frame_stack
        )
    ...
    def validation_step(self, batch, batch_idx, namespace="validation"):
        if self.guidance_scale:
            self.interact(
                batch, namespace=namespace, guidance_scale=self.guidance_scale
            )
    ...
    def interact(self, batch, namespace="validation", guidance_scale=0.0):
        ...
        while not terminate and steps < n_frames:
            ...
            plan = [
                torch.randn(
                    (batch_size,) + tuple(self.x_stacked_shape), device=self.device
                )
            ...
            plan_history_m = []
            pyramid_height = self.sampling_timesteps + int(
            ...
            for m in range(pyramid_height):
                ...
                with torch.enable_grad():
                    for t in range(horizon):
                        ...
                        if guidance_scale:
                            for _ in range(self.ddim_repeats - 1):
                                ...
                                time = torch.full((batch_size,), time_pairs[i][0]).to(
                                    self.device
                                )
                                bundle_temp = self.transition_model.q_sample(
                                    bundle_clean, time
                                )
                    ...
                    if guidance_scale:
                        clean_plan_samples = torch.stack(
                            [torch.stack(sample) for sample in clean_plan_samples]
                        )
                        ...
                        weight = (
                            torch.from_numpy(np.array(weight)).float().to(self.device)
                        )
                        ...
                        episode_return_dense = (
                            r_scale
                            * clean_plan_samples[..., -1]
                            * weight[:, None, None]
                        )
                        episode_return_dense = (
                            episode_return_dense.sum([0, 1]).mean() * 4.0
                        )

                        # calculate negative distance to goal, so we can guide by goal like original diffusion planning paper
                        unnormalized_plan_samples = self._unnormalize_x(
                            clean_plan_samples
                        )
                        dist = torch.linalg.norm(
                            unnormalized_plan_samples[..., :2] - goal[None, :, None],
                            dim=-1,
                        )
                        episode_return_sparse = (
                            -(g_scale * n_frames * dist * weight[:, None, None])
                            .sum(1)
                            .mean()
                        )
                        episode_return = (
                            episode_return_dense
                            if self.use_reward
                            else episode_return_sparse
                        )

                        # guidance by episode return
                        grads = torch.autograd.grad(
                            [episode_return], last_plan, allow_unused=True
                        )
                        for t in range(horizon):
                            plan[t] = (
                                plan[t] + guidance_consts[t] * grads[t] * guidance_scale
                            )
            ...
            for bundle in plan_history[-1][-1][: self.open_loop_horizon]:
                ...
                obs, reward, done, info = envs.step(
                    np.nan_to_num(action.detach().cpu().numpy())
                )
                ...
                bundle = self.make_bundle(obs, action, reward[..., None]).to(
                    self.device
                )
            ...
            if context_frames > 0:
                ...
                context = context.view(
                    context_frames // self.frame_stack, self.frame_stack, batch_size, -1
                )
                context = context.permute(0, 2, 1, 3).contiguous()
                context = context.reshape(
                    context_frames // self.frame_stack, batch_size, -1
                )
                context_condition = None  # conditions[steps - context_frames : steps]
                for bundle, condition in zip(context, context_condition):
                    z, _, _, _ = self.transition_model(
                        z, bundle, condition, deterministic_t=0
                    )
        ...
        images = make_trajectory_images(
            self.env_id, trajectory, batch_size, start, goal
        )
        ...
        if self.debug:
            ...
            if "medium" in self.env_id:
                indicies = [
                    2,
                    4,
                ]  # choose start/goal with relatively long distances (harder)
            else:
                indicies = [
                    2,
                    7,
