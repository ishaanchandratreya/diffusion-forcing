...
class Maze2dOfflineRLDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        value = torch.from_numpy(
            self.dataset["values"][idx : idx + self.n_frames]
        ).float()
    ...
    def get_dataset(self):
        ...
        if self.dataset_url is not None:
            ...
            h5path = os.path.join(self.save_dir, self.cfg.env_id + ".hdf5")
        data_dict = {}
        ...
        observation_mean = data_dict["observations"].mean(axis=0)
        observation_std = data_dict["observations"].std(axis=0)
        action_mean = data_dict["actions"].mean(axis=0)
        action_std = data_dict["actions"].std(axis=0)

        reward_mean = data_dict["rewards"].mean()
        reward_std = data_dict["rewards"].std()

        # make dictionary of evaluated stats call stat_dict
        stat_dict = {
...
# This environment and associated gym environment ("with dist") suffix returns the distance on index 3
class Maze2dOfflineDatasetWithDist(Maze2dOfflineRLDataset):
    def __init__(self, cfg: DictConfig, split: str = "training"):
    ...
    def get_dataset(self):
        ...
        if self.dataset_url is not None:
            ...
            h5path = os.path.join(self.save_dir, self.cfg.data_file_name)
        data_dict = {}
        with h5py.File(h5path, "r") as dataset_file:
            for k in get_keys(dataset_file):
                try:  # first try loading as an array
        ...
        data_dict["observations"] = np.concatenate(
            (data_dict["observations"][:, :2], data_dict["dist_to_wall"][:, None]),
            axis=-1,
        )
...
def test_base_dataset():
    ...
    os.chdir("../..")
    cfg = MagicMock()
    cfg.env_id = "maze2d-custom-v1"
    cfg.episode_len = 600
    cfg.gamma = 1.0

    cfg.save_dir = "/home/chandratreya/mit/diffusion-forcing/data/maze2d"
    cfg.dataset_url = None
    cfg.observation_shape = [4]
    cfg.action_dim = 2
    cfg.goal_dim = 2
...
def test_dataset_with_dist():
    ...
    cfg.save_dir = "/home/chandratreya/mit/diffusion-forcing/data/maze2d"
    cfg.data_file_name = "maze2d-custom-v1-unblocked_dist.hdf5"
    cfg.dataset_url = None
    cfg.observation_shape = [5]
    cfg.action_dim = 2
    cfg.goal_dim = 2

    dataset = Maze2dOfflineDatasetWithDist(cfg)
    o, a, r, n = dataset.__getitem__(10)
    print(o.shape, a.shape, r.shape, n.shape)
    plt.figure()
    plt.scatter(o[:, 0], o[:, 1], c=np.arange(len(o)), cmap="Reds")
